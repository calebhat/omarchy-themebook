...
const upd = m.parseGitUpdateLine(JSON.stringify({ event: "theme", slug: "nord", status: "available", detail: "HEAD abc origin def" }))
if (!upd || upd.slug !== "nord" || upd.status !== "available") throw new Error("theme line")
if (m.parseGitUpdateLine(JSON.stringify({ event: "theme", slug: "nord", status: "updated" }))) throw new Error("reject apply status")
if (m.parseGitUpdateLine(JSON.stringify({ event: "theme", slug: "../x", status: "available" }))) throw new Error("reject bad slug")
if (!m.hasGitThemes([{ slug: "nord", git: true }]) || m.hasGitThemes([{ slug: "nord" }])) throw new Error("hasGit")
if (m.gitUpdateSummary([]) !== "No git-installed user themes to check.") throw new Error("empty summary")
if (m.gitUpdateSummary([{ status: "available" }, { status: "current" }, { status: "failed" }]).indexOf("failed") < 0)
  throw new Error("summary mix")
if (m.gitUpdateSummary([{ status: "available" }]).indexOf("update available") < 0) throw new Error("available summary")
