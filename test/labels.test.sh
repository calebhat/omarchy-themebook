...
need=(
  ...
  'text: "Delete folder'
  'F Favorite   H Hide   U Check git   Del Delete from OS   Shift+↑/↓ Sort in folder'
...
if ! grep -F -q -- 'Checking git themes' "$qml"; then
  echo "missing: git check progress overlay" >&2
  exit 1
fi
if grep -E -q 'git[^\n]*\b(pull| merge | checkout | reset )' "$root/scripts/update-git"; then
  echo "git helper still applies remote content" >&2
  exit 1
fi
